...
- Keep device identifiers as a collection in the model, but serialize Home Assistant's minimal accepted form: one identifier as a scalar and any other non-empty count as a list.
...
- Compare parsed JSON semantically: ignore object-property order and formatting, compare JSON numbers by mathematical value, and require array order, other values, and property presence to match. Canonicalize a one-item `device.identifiers` list to its minimal scalar form. Keep any other narrowly accepted normalization explicit in the individual test case and link it to a tracking issue.
